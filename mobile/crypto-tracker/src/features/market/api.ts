...
export async function fetchCoinChart(coinId: string, currency: Currency): Promise<number[]> {
  const response = await fetch(`https://api.coingecko.com/api/v3/coins/${coinId}/market_chart?vs_currency=${currency.toLowerCase()}&days=30`);
