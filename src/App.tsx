...
const experience = [
  ['2025 — сейчас', 'Frontend development', 'Изучаю продуктовый подход и превращаю идеи в работающие интерфейсы, уделяя внимание структуре и деталям.'],
  ['2024 — 2025', 'Погружение в веб', 'Освоил HTML, CSS, JavaScript и базовые принципы UX. Начал собирать первые страницы и разбирать чужие интерфейсы.'],
  ['Сейчас', 'Открыт к первым задачам', 'Ищу команду или проект, где смогу расти рядом с сильными разработчиками и приносить пользу уже на старте.'],
];
  ...
  return (
    <div className="site-shell">
      ...
      <main id="main">
        <section id="hero" className="hero section-pad" aria-labelledby="hero-title">
          ...
          <div className="hero-grid container">
            <div className="hero-copy">
              <Reveal><p className="eyebrow"><span className="status-dot" /> Никита / открыт к новым задачам</p></Reveal>
              <Reveal><h1 id="hero-title">Собираю цифровые<br /><span>впечатления.</span></h1></Reveal>
              <Reveal><p className="hero-lede">Я <strong>{roleText}</strong><span className="typing-cursor" aria-hidden="true">|</span>. Проектирую спокойные, выразительные сайты и интерфейсы, в которых бизнесу легко расти, а людям — легко ориентироваться.</p></Reveal>
              <Reveal className="hero-actions"><a className="button button--primary" href="#about">Узнать обо мне <ArrowDown /></a><a className="button button--ghost" href="#contact">Давайте знакомиться <ArrowUpRight /></a></Reveal>
              <Reveal><div className="hero-meta"><span>01 / 04</span><span>React · TypeScript · CSS</span></div></Reveal>
            </div>
        ...
        <div className="signal-bar"><div className="container signal-bar__inner"><span>Портфолио / 2026</span><span className="signal-hide-mobile">Дизайн · разработка · внимание к деталям</span><span><i /> открыт к диалогу</span></div></div>
        ...
        <section id="about" className="section-pad section-about" aria-labelledby="about-title">
          <div className="container about-grid">
            ...
            <Reveal className="about-content"><div className="portrait-card"><div className="portrait-card__grid" /><div className="portrait-card__avatar">К</div><span className="portrait-card__caption">Никита<br />frontend / 2026</span><span className="portrait-card__orb" /></div><div className="about-copy"><p className="lead">Я начинающий frontend-разработчик, который любит превращать сложные мысли в ясные интерфейсы.</p><p>Мне важны не только аккуратные пиксели, но и то, что за ними: сценарий пользователя, ритм страницы, понятная логика и ощущение, что всё находится на своём месте.</p><div className="about-facts"><div><strong>01</strong><span>портфолио<br />в работе</span></div><div><strong>∞</strong><span>любопытства<br />к новым задачам</span></div><div><strong>01</strong><span>цель — расти<br />в сильной команде</span></div></div></div></Reveal>
          </div>
        </section>
        ...
        <section id="experience" className="section-pad section-muted" aria-labelledby="experience-title">
          <div className="container experience-grid"><Reveal><p className="eyebrow">03 / путь</p><h2 id="experience-title">Расту через<br /><span>практику.</span></h2><p className="section-intro">Сейчас я ищу первые реальные задачи и людей, рядом с которыми смогу стать сильнее.</p></Reveal><Reveal><ol className="timeline">{experience.map(([date, title, text], index) => <li key={title}><span className="timeline__number">0{index + 1}</span><div><time>{date}</time><h3>{title}</h3><p>{text}</p></div></li>)}</ol></Reveal></div>
        </section>

        <section className="section-pad quote-section" aria-label="Отзыв о подходе"><div className="container"><Reveal className="quote-card"><span className="quote-mark">“</span><blockquote>Хороший интерфейс не пытается понравиться всем. Он точно знает, кому помогает.</blockquote><footer><span className="quote-avatar">А</span><span><strong>Александра Морозова</strong><small>product designer / concept review</small></span></footer></Reveal></div></section>

        <section id="contact" className="section-pad contact-section" aria-labelledby="contact-title"><div className="container contact-grid"><Reveal><p className="eyebrow">04 / контакт</p><h2 id="contact-title">Есть задача?<br /><span>Давайте обсудим.</span></h2><p className="contact-intro">Расскажи, что нужно сделать. Даже если идея пока сырая — разложим её на понятные шаги.</p><div className="contact-links"><a href="mailto:hello@kosmik.dev"><MailIcon />hello@kosmik.dev</a><a href="https://t.me/fr_code" target="_blank" rel="noreferrer"><TelegramIcon />@fr_code</a><a href="https://github.com" target="_blank" rel="noreferrer"><GithubIcon />GitHub</a></div></Reveal><Reveal><form className="contact-form" onSubmit={handleSubmit} noValidate><div className="form-row"><label>Имя<input type="text" value={form.name} onChange={(event) => updateField('name', event.target.value)} placeholder="Как к тебе обращаться?" aria-invalid={Boolean(formErrors.name)} aria-describedby={formErrors.name ? 'name-error' : undefined} />{formErrors.name && <small id="name-error" className="form-error">{formErrors.name}</small>}</label><label>Email<input type="email" value={form.email} onChange={(event) => updateField('email', event.target.value)} placeholder="you@example.com" aria-invalid={Boolean(formErrors.email)} aria-describedby={formErrors.email ? 'email-error' : undefined} />{formErrors.email && <small id="email-error" className="form-error">{formErrors.email}</small>}</label></div><label>О задаче<textarea value={form.message} onChange={(event) => updateField('message', event.target.value)} placeholder="Коротко о проекте, сроках и том, что важно" rows={5} aria-invalid={Boolean(formErrors.message)} aria-describedby={formErrors.message ? 'message-error' : undefined} />{formErrors.message && <small id="message-error" className="form-error">{formErrors.message}</small>}</label><div className="form-submit"><button className="button button--primary" type="submit">{formSent ? 'Сообщение проверено' : 'Отправить сообщение'} <ArrowUpRight /></button>{formSent && <p className="form-success" role="status">Спасибо. Я свяжусь с тобой в ближайшее время.</p>}</div></form></Reveal></div></section>
      </main>
